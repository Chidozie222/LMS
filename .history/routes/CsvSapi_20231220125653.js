    ...
    try {
        if (!req.file) {
            ...
            const csvfile = async (item) => {
                let UserByClass = await SAPI.find({ SchoolEmail: item.SchoolEmail, Class: item. })
                let UserBySchoolEmail = await SAPI.find({ SchoolEmail: item.SchoolEmail, StudentFirstName })
                let UserByStudentUsername = await SAPI.find({ SchoolEmail: item.SchoolEmail, StudentUsername })
                let UserByParentUserName = await SAPI.find({ SchoolEmail: item.SchoolEmail, ParentUserName })
                let UserByParentEmail = await SAPI.find({ SchoolEmail: item.SchoolEmail, ParentEmail })
                let UserByStudentEmail = await SAPI.find({ SchoolEmail: item.SchoolEmail, StudentEmail })
                let UserByClassCapacity = await Classes.find({ SchoolEmail: item.SchoolEmail, Class })
