    ...
    try {
        if (!req.file) {
            ...
            const csvfile = async (item) => {
                let UserByClass = await SAPI.find({ SchoolEmail: item.S, Class })
                let UserBySchoolEmail = await SAPI.find({ SchoolEmail: item.S, StudentFirstName })
                let UserByStudentUsername = await SAPI.find({ SchoolEmail: item.S, StudentUsername })
                let UserByParentUserName = await SAPI.find({ SchoolEmail: item.S, ParentUserName })
                let UserByParentEmail = await SAPI.find({ SchoolEmail: item.S, ParentEmail })
                let UserByStudentEmail = await SAPI.find({ SchoolEmail: item.S, StudentEmail })
                let UserByClassCapacity = await Classes.find({ SchoolEmail: item.S, Class })
