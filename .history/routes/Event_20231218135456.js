...
Event.post('/Event', async (req, res) => {
    ...
    try {
        ...
        if (UserByStartDateAndTitle.length > 0) {
            res.send({ status: 'error', message: 'Event has already been set, Looking for another starting date' })
        } else {
            await Events.create({
                StartDate, StartTime, EndDate, EndTime, Title, Description, ShowToAll, SchoolEmail
            })
        }
    } catch (error) {
