...
Te.use(express.static('public'))


Te.post('/student_and_parent', upload.fields([{ name: 'StudentPicture', maxCount: 1 }, { name: 'ParentPicture', maxCount: 1 }]), async (req, res) => {
    ...
    try {
        let UserBySchoolEmail = await Te.find({ SchoolEmail, StudentFirstName })
        let UserByStudentUsername = await Te.find({ SchoolEmail, StudentUsername })
        let UserByParentUserName = await Te.find({ SchoolEmail, ParentUserName })
        let UserByParentEmail = await Te.find({ SchoolEmail, ParentEmail })
        let UserByStudentEmail = await Te.find({ SchoolEmail, StudentEmail })
