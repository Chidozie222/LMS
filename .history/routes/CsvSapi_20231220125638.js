    ...
    try {
        if (!req.file) {
            ...
            const csvfile = async (item) => {
                let UserByClass = await SAPI.find({ SchoolEmail: item, Class })
                let UserBySchoolEmail = await SAPI.find({ SchoolEmail: item, StudentFirstName })
                let UserByStudentUsername = await SAPI.find({ SchoolEmail: item, StudentUsername })
                let UserByParentUserName = await SAPI.find({ SchoolEmail: item, ParentUserName })
                let UserByParentEmail = await SAPI.find({ SchoolEmail: item, ParentEmail })
                let UserByStudentEmail = await SAPI.find({ SchoolEmail: item, StudentEmail })
                let UserByClassCapacity = await Classes.find({ SchoolEmail: item, Class })
