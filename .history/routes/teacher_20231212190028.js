...
Teacher.post('/student_and_parent', upload.fields([{ name: 'StudentPicture', maxCount: 1 }, { name: 'ParentPicture', maxCount: 1 }]), async (req, res) => {
    ...
    try {
        let UserBySchoolEmail = await Teacher.find({ SchoolEmail, StudentFirstName })
        let UserByStudentUsername = await Teacher.find({ SchoolEmail, StudentUsername })
        let UserByParentUserName = await Teacher.find({ SchoolEmail, ParentUserName })
        let UserByParentEmail = await Teacher.find({ SchoolEmail, ParentEmail })
        let UserByStudentEmail = await Teacher.find({ SchoolEmail, StudentEmail })
