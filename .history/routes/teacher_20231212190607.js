...
Teacher.post('/student_and_parent', upload.fields([{ name: 'StudentPicture', maxCount: 1 }, { name: 'ParentPicture', maxCount: 1 }]), async (req, res) => {
    ...
    try {
        let UserBySchoolEmail = await T.find({ SchoolEmail, StudentFirstName })
        let UserByStudentUsername = await T.find({ SchoolEmail, StudentUsername })
        let UserByParentUserName = await T.find({ SchoolEmail, ParentUserName })
        let UserByParentEmail = await T.find({ SchoolEmail, ParentEmail })
        let UserByStudentEmail = await T.find({ SchoolEmail, StudentEmail })
