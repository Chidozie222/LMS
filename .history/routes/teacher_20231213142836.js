...
Teacher.post('/teachers', upload.single('TeacherPicture'), async (req, res) => {
    ...
    try {
        ...
        let UserByUsername = await Teachers.find({ SchoolEmail, TeacherUsername })
        let UserByParentEmail = await Teachers.find({ SchoolEmail, TeacherEmail })

        if (UserBySchoolEmail.length > 0 && UserByStudentUsername.length > 0 && UserByParentEmail.length > 0) {
